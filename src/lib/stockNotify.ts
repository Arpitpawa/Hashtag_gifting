...
// Emails everyone still waiting on the "notify me when back in stock" list
// for a product, once it's actually back in stock. Shared by:
//   - the admin product PUT/PATCH route (auto-fires whenever stock goes
//     from 0 to something, no matter which admin screen changed it)
//   - /api/admin/stock-notify (kept as a manual retry endpoint, in case an
//     earlier auto-send partially failed)
// Only ever emails a given signup once — sendEmail success flips
// `notified` to true, so re-running this for the same product afterward
// finds nothing left to send.
export async function notifyBackInStock(productId: number): Promise<{ sent: number; product?: string }> {
  ...
  return { sent, product: product.name };
}
