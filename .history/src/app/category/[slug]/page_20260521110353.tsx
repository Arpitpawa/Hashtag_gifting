...
export default function CategoryClient({
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (categoryId) params.set("category", slug);
  ...
  }, [slug, categoryId, sort, minPrice, maxPrice, page, searchQ]);
      ...
      <div className="bg-white border-b border-[#e8e0d5]">
        <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10 py-8">
          ...
          <nav className="flex items-center gap-2 text-[12px] text-[#aaa] mb-4 flex-wrap">
            ...
            <span>/</span>
            <span className="text-[#1a1a1a] font-medium capitalize">{displayName}</span>
          </nav>
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Link href="/shop"
                className="text-[12px] font-semibold text-[#c0555a] hover:underline underline-offset-2">
                View all gifts →
              </Link>
            </div>
          </div>
