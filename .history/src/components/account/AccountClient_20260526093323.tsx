    ...
    return (
      <div className="bg-white rounded-2xl border border-[#e8e0d5] p-6 flex flex-col gap-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-20 bg-[#e8e0d5] rounded-2xl animate-pulse"
          />
        ))}
      </div>
    );
  ...
  if (orders.length === 0)
    return (
      <div className="bg-white rounded-2xl border border-[#e8e0d5] p-12 text-center">
        <ShoppingBag
          size={40}
          className="text-[#e8e0d5] mx-auto mb-4"
          strokeWidth={1.5}
        />
        ...
        <Link
          href="/shop"
          className="inline-flex items-center gap-2 px-6 py-3 bg-[#c0555a] text-white text-[13px] font-bold rounded-full hover:bg-[#a84449] transition-all"
        >
  ...
  return (
    <div className="flex flex-col gap-4">
      {orders.map((order) => (
        <div
          key={order.id}
          className="bg-white rounded-2xl border border-[#e8e0d5] p-5"
        >
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span
                className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${statusColor(order.paymentStatus)}`}
              >
                {order.paymentStatus}
              </span>
              <span
                className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${statusColor(order.deliveryStatus)}`}
              >
                {order.deliveryStatus}
              </span>
              <span className="text-[14px] font-bold text-[#c0555a]">
          ...
          >
            {order.items.slice(0, 4).map((item, i) => (
              <div
                key={i}
                className="relative w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-[#f8f5f0] border border-[#e8e0d5]"
              >
                <Image
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <a
              href={`https://wa.me/917665909909?text=Hi! My order ID is %23${order.id}. Can you help me track it?`}
              target="_blank"
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-[#e8e0d5] p-6 flex flex-col gap-3">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="h-24 bg-[#e8e0d5] rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : (
        <>
          {addresses.map((addr) => (
            <div
              key={addr.id}
              className="bg-white rounded-2xl border border-[#e8e0d5] p-5 flex items-start justify-between gap-4"
            >
              <div className="flex items-start gap-3">
                <Home
                  size={16}
                  className="text-[#c0555a] mt-0.5 flex-shrink-0"
                />
                <div>
                  ...
                  <p className="text-[12px] text-[#888] mt-1 flex items-center gap-1">
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="aspect-square bg-[#e8e0d5] rounded-2xl animate-pulse"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      {items.map((item) => (
        <div
          key={item.id}
          className="group relative bg-white border border-[#e8e0d5] rounded-2xl overflow-hidden"
        >
          <Link href={`/product/${item.product.slug}`}>
            ...
            <div className="p-3">
              <p className="text-[13px] font-semibold text-[#1a1a1a] capitalize line-clamp-2 leading-snug">
                {item.product.name}
              </p>
              <p className="text-[14px] font-bold text-[#c0555a] mt-1">
