...
export default function ProductForm({ mode, productId }: Props) {
  ...
  // ── Category tree — API returns parents with children nested ─────────────────
        ...
        <div className="flex flex-col gap-5">
          ...
          <Section title="Product specifications">
            <p className="text-[12px] text-[#888] mb-4 leading-relaxed">
              These show in the "Product Details" tab on the product page. Add details like Material, Size, Care instructions etc.
            </p>
              ...
              {specs.map((spec, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input value={spec.label}
                    ...
                    className="w-32 border border-[#e8e8e8] rounded-xl px-3 py-2 text-[13px] outline-none focus:border-[#c0555a] flex-shrink-0" />
                  <input value={spec.value}
                    onChange={e => setSpecs(p => p.map((s,j) => j===i ? {...s, value: e.target.value} : s))}
                    placeholder="e.g. Premium ceramic"
                    className="flex-1 border border-[#e8e8e8] rounded-xl px-3 py-2 text-[13px] outline-none focus:border-[#c0555a]" />
          ...
          {/* Personalisation */}
          <Section title="Personalisation fields">

            {/* Simple guide */}
            <div className="bg-[#f3efe8] rounded-xl p-4 mb-5 text-[13px] text-[#555] leading-relaxed">
              <p className="font-bold text-[#1a1a1a] mb-2">How personalisation works:</p>
              <p className="mb-1.5">When a customer opens this product page, they see a form to fill before adding to cart. You define what fields they fill.</p>
              <p className="font-semibold text-[#1a1a1a] mb-1">Common examples:</p>
              <ul className="space-y-1 ml-3">
                <li>• <span className="font-medium">Name on mug</span> — add a "Text input" field labelled "Enter name"</li>
                <li>• <span className="font-medium">Photo on cushion</span> — add a "Photo upload" field labelled "Upload photo"</li>
                <li>• <span className="font-medium">Birthday message</span> — add a "Long text" field labelled "Add a message"</li>
              </ul>
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 flex-1">
                      <div>
                        ...
                        <select value={field.type}
                          ...
                          <option value="text">Text (short — name, number)</option>
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-[#aaa] uppercase mb-1">Label (shown to customer)</p>
                      ...
                      <label className="flex items-center gap-2 text-[13px] text-[#555] cursor-pointer col-span-2">
                        <input type="checkbox" checked={field.required}
                          onChange={e => setCustFields(p => p.map((f,j) => j===i ? {...f, required: e.target.checked} : f))}
                          className="accent-[#c0555a]" />
                        <span>Required — customer must fill this before adding to cart</span>
                      </label>
                    </div>
          ...
          {/* Live preview setup — only when customizable */}
          {customizable && (
            <Section title="Live canvas preview setup">
              <div className="bg-[#f3efe8] border border-[#e8e0d5] rounded-xl p-4 mb-4 text-[12px] text-[#555] leading-relaxed">
                <p className="font-bold text-[#1a1a1a] mb-1">Optional — adds a live preview modal for customers</p>
                <p>1. Upload a clean product photo (no design) to Cloudinary → copy the URL → paste below as Template URL</p>
                <p>2. Set zones as JSON — tells the system where to overlay customer's photo/text on the template</p>
                <p className="mt-1 font-medium">Example for a mug: <code className="bg-white px-1 rounded text-[11px]">[{`{"id":"photo","type":"image","x":120,"y":80,"width":260,"height":200,"label":"Your photo"}`}]</code></p>
              </div>
              <div className="flex flex-col gap-3">
                <Field label="Template image URL (clean product photo from Cloudinary)">
                  <input value={previewTemplate} onChange={e => setPreviewTemplate(e.target.value)}
                    placeholder="https://res.cloudinary.com/your-cloud/image/upload/mug-template.jpg"
                    className={inputCls()} />
                </Field>
                <Field label="Preview zones (JSON array — defines where photo/text goes)">
                  <textarea value={previewZones} onChange={e => setPreviewZones(e.target.value)}
                    rows={5} placeholder={`[\n  {"id":"photo","type":"image","x":120,"y":80,"width":260,"height":200,"label":"Your photo"},\n  {"id":"name","type":"text","x":100,"y":320,"width":300,"height":40,"fontSize":22,"label":"Your name"}\n]`}
                    className={`${inputCls()} font-mono text-[12px] resize-none`} />
                  {previewZones && (() => { try { JSON.parse(previewZones); return <p className="text-[11px] text-green-600 flex items-center gap-1">✓ Valid JSON</p>; } catch { return <p className="text-[11px] text-red-500">⚠ Invalid JSON — check syntax</p>; } })()}
                </Field>
              </div>
            </Section>
          )}
        ...
        <div className="flex flex-col gap-5">
          ...
          <Section title="Category">
            <div className="flex flex-col gap-1 max-h-[400px] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
              {errors.category && <p className="text-[11px] text-red-500 mb-2">{errors.category}</p>}
