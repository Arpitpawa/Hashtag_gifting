...
export default function WoodenCorporateGifting() {
  ...
  const toggleWishlist = (e: React.MouseEvent, index: number) => {
    e.preventDefault();
    setWishlist(prev => prev.includes(index) ? prev.filter(i => i !== index) : [...prev, index]);
  };

  return (
    <section className="pt-0 pb-16 md:pb-5">
      <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10">
        ...
        <div className="text-center mb-10 md:mb-12">
          ...
          <p className="text-black text-[14px] md:text-[15px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-10">
          {visibleProducts.map((product, i) => (
            <Link key={i} href={product.link} className="group block">
              <div className="relative overflow-hidden rounded-lg bg-[#f5f0ea] mb-4" style={{ aspectRatio: "3/3.5" }}>
                <Image src={product.image} alt={product.name} fill
                  className="object-cover transition-transform duration-700 group-hover:scale-105"
                  sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" />
                <button onClick={e => toggleWishlist(e, i)}
                  ...
                  <Heart size={15} className={wishlist.includes(i) ? "fill-[#c0555a] text-[#c0555a]" : "text-gray-400"} />
                </button>
              </div>
              <div>
                <h3 className="text-[14px] md:text-[15px] font-medium text-[#1a1a1a] mb-0.5 group-hover:text-[#c0555a] transition-colors duration-200">
                  {product.name}
                </h3>
                <p className="text-[12px] text-[#999] mb-0.5">{product.category}</p>
