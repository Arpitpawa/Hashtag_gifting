          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center w-full">
            ...
            <div className={`transition-all duration-500 ${animating ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}>
              ...
              <h1
                ...
                style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(40px, 5vw, 68px)" }}
              >
                {slide.heading}
                <br />
            ...
            {slide.discount && (
              <div className={`hidden lg:grid grid-cols-2 gap-3 transition-all duration-500 ${animating ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}>
                {slide.discount.map((d, i) => (
                  <div key={i} className="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-5 flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-[#c0555a]/20 flex items-center justify-center flex-shrink-0">
                      <Gift size={18} className="text-[#c0555a]" strokeWidth={1.8} />
                    </div>
                    <div>
                      <p className="text-white font-bold text-[18px]">{d.percent}</p>
        ...
        <div className="flex overflow-hidden">
          <div className="flex animate-marquee whitespace-nowrap">
            {[
              ...
              <span key={i} className="inline-flex items-center gap-2 mx-8 text-[12px] font-medium text-white/90">
                ✦ {text}
              </span>
            ))}
          </div>
