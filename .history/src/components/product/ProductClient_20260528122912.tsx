...
export default function ProductClient({ product }: { product: Product }) {
  ...
  return (
    <div className="min-h-screen bg-[#f3efe8]">
      ...
      <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-12 mb-20">
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* 4. Customizable — always just a button, modal opens on click */}
            {product.customizable && product.customizationFields?.length > 0 && (
              <div id="product-customizer">
                <button
                  onClick={() => setLivePreview(true)}
                  disabled={product.stock === 0}
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-full text-[15px] font-bold bg-[#c0555a] text-white hover:bg-[#a84449] disabled:opacity-50 transition-all shadow-md">
                  {product.stock === 0 ? "Out of stock" : "✨ Personalise & Add to Cart"}
                </button>
                {product.stock === 0 && <NotifyMeButton productId={product.id} />}
              </div>
            )}
      ...
      {/* Live preview / personalisation modal — always modal for customizable products */}
      {product.customizable && product.customizationFields?.length > 0 && (
        <LivePreviewModal
          open={livePreview}
          ...
          previewTemplate={product.previewTemplate || product.images?.[0] || ""}
          previewZones={(product.previewZones as any[]) || []}
          customFields={product.customizationFields || []}
          adding={adding}
          onAddToCart={async (customization) => {
            setAdding(true);
            try {
              await addToCart(product.id, 1, customization);
