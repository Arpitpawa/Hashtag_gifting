      ...
      <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-12 mb-20">
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* 4. Customizable — live preview modal OR standard customizer */}
            {product.customizable && product.customizationFields?.length > 0 && (
              <div id="product-customizer">
                {product.previewTemplate ? (
                  // Has template → show "Personalise & Add to Cart" button that opens live modal
                  <button
                    onClick={() => setLivePreview(true)}
                    disabled={product.stock === 0}
                    className="w-full flex items-center justify-center gap-2 py-4 rounded-full text-[15px] font-bold bg-[#c0555a] text-white hover:bg-[#a84449] disabled:opacity-50 transition-all">
                    ✨ Personalise &amp; Add to Cart
                  </button>
                ) : (
                  // No template → standard form-based customizer
                  <ProductCustomizer
