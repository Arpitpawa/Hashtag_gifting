  ...
  return (
    <>
      <header className="bg-white relative z-50">
        ...
        {/* ── MAIN NAVBAR ROW — logo + switcher + icons all in one ── */}
        <div className="border-b border-[#ececec]">
          <div className="container-custom h-[75px] flex items-center justify-between">

            {/* LEFT — switcher pills (desktop) + hamburger (mobile) + search/user */}
              ...
              <Sheet>
                ...
                <SheetContent side="left" className="w-[300px] bg-[#f7f4ef] p-0">
                  ...
                  <div className="flex gap-2 px-4 py-3 border-b border-[#ececec]">
                    <Link href="/" className={`flex-1 text-center py-2 rounded-full text-[11px] font-semibold border transition-all ${!isCorporate ? "bg-[#c0555a] text-white border-[#c0555a]" : "bg-white text-[#c0555a] border-[#c0555a]"}`}>
                      Personal gifts
                    </Link>
                    <Link href="/corporate" className={`flex-1 text-center py-2 rounded-full text-[11px] font-semibold border transition-all ${isCorporate ? "bg-[#c0555a] text-white border-[#c0555a]" : "bg-white text-[#c0555a] border-[#c0555a]"}`}>
                      Corporate
                    </Link>
                  </div>
            ...
            {navItems.map((item) => (
              ...
              >
                <button className={`flex items-center gap-1 text-[11px] font-medium tracking-wide transition-all duration-200 pb-0.5 border-b-2 whitespace-nowrap ${
                  activeMenu === item.title ? "text-black border-black" : "text-[#555] border-transparent hover:text-black hover:border-gray-300"
                }`}>
                  {item.title}
                ...
                {activeMenu === item.title && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 z-50">
                    <div className="bg-white border-x border-b border-[#ececec] shadow-[0_16px_48px_rgba(0,0,0,0.08)] overflow-hidden"
                      ...
                      <div className="bg-[#f7f4ef] px-8 py-3.5 border-b border-[#ececec] flex items-center justify-between">
                        ...
                        <Link href={`/category/${item.title.toLowerCase().replace(/\s+/g, "-")}`} className="text-[11px] text-[#c0555a] font-medium flex items-center gap-1 hover:underline">
                          View all <ArrowRight size={10} />
                        </Link>
                      </div>
                      <div className={`px-8 py-5 grid gap-x-12 gap-y-0 ${item.items.length > 8 ? "grid-cols-3" : "grid-cols-2"}`}>
                        {item.items.map((subItem) => (
                          <Link key={subItem} href={`/category/${subItem.toLowerCase().replace(/\s+/g, "-")}`}
          ...
          <div className="relative z-10 bg-white shadow-2xl">
            <div className="container-custom py-4 flex items-center gap-4">
              ...
              <input ref={inputRef} type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={displayedPlaceholder} className="flex-1 text-[15px] outline-none text-gray-800 placeholder:text-gray-400 bg-transparent" />
              {searchQuery && <button onClick={() => setSearchQuery("")} className="text-gray-400 hover:text-black transition-colors"><X size={17} /></button>}
              <button onClick={() => setSearchOpen(false)} className="text-[13px] text-gray-500 hover:text-black transition-colors font-medium ml-2">Cancel</button>
            </div>
