        ...
        <div className="container-custom h-[70px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            <Sheet>
              ...
              <SheetContent side="left" className="w-[300px] bg-[#f7f4ef]">
                <div className="mt-10 flex flex-col gap-6">
                  {navItems.map((item) => (
                    <button key={item.title} className="text-left text-[15px]">
                      {item.title}
                    </button>
                  ))}
                </div>
            ...
            <div className="hidden lg:flex items-center gap-5">
              <button><Search size={21} strokeWidth={1.5} /></button>
              <button><User size={21} strokeWidth={1.5} /></button>
            </div>
          </div>
          ...
          {/* Right */}
          <div className="flex items-center gap-5">
            <button><Heart size={21} strokeWidth={1.5} /></button>
      ...
      <div className="hidden lg:block border-b border-[#ececec] bg-white">
        <div className="container-custom h-[62px] flex items-center justify-center gap-10">
          {navItems.map((item) => (
            ...
            >
              <button className="flex items-center gap-1.5 text-[15px] text-[#444] hover:text-black transition-all duration-200">
                {item.title}
              ...
              {activeMenu === item.title && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 z-50">
                  <div className="w-[350px] bg-[#f8f8f8] border border-[#ececec] rounded-b-[5px] shadow-[0_8px_24px_rgba(0,0,0,0.08)] px-8 py-7">
                    <div className="grid grid-cols-2 gap-x-10 gap-y-7">
                      {item.items.map((subItem) => (
                        <button
                          key={subItem}
                          className="text-left text-[15px] text-[#555] leading-none hover:text-black transition-all duration-200"
                        >
                          {subItem}
                        </button>
                      ))}
