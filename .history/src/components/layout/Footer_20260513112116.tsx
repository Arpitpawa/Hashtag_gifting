...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: NAVY }} className="text-white">
      ...
      {/* ── TOP STRIP ── */}
      <div style={{ backgroundColor: NAVY_LIGHT }} className="py-4 px-4 border-b border-white/5">
        <div className="max-w-[1450px] mx-auto flex flex-col md:flex-row items-center justify-center gap-3 md:gap-12 text-center">
          {[
            ...
            <span key={i} className="flex items-center gap-2 text-[12px] text-white/70 font-medium">
              <span className="text-[#c4922a]">{item.icon}</span>
              {item.text}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10 pt-14 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            {/* LOGO — Poppins style matching navbar */}
            <Link href="/" className="flex flex-col leading-none select-none mb-5 w-fit">
              ...
              <span className="text-[9px] font-semibold tracking-[5px] text-[#c4922a] uppercase -mt-1">
                gifting
              </span>
            </Link>
            ...
            <div className="flex flex-col gap-3 mb-8">
                ...
                className="flex items-start gap-2.5 text-[13px] text-white/50 hover:text-white transition-colors"
              >
                <MapPin size={14} className="mt-0.5 flex-shrink-0 text-[#c4922a]" />
              ...
              >
                <Phone size={14} className="flex-shrink-0 text-[#c4922a]" />
                +91 86400 30112
              </a>
                ...
                className="flex items-center gap-2.5 text-[13px] text-white/50 hover:text-white transition-colors"
              >
                <Mail size={14} className="flex-shrink-0 text-[#c4922a]" />
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                { icon: <YouTubeIcon size={16} />, href: "https://youtube.com/@hashtagifting", label: "YouTube" },
                  ...
                  className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-white hover:border-white/40 hover:bg-white/10 transition-all duration-300"
                >
                  {social.icon}
                </a>
              ))}
          ...
          {footerLinks.map((col, i) => (
            <div key={i} className="lg:col-span-1">
              <h4 className="text-[11px] font-semibold uppercase tracking-[2px] text-white/30 mb-5">
                {col.title}
              </h4>
              <ul className="flex flex-col gap-3">
                {col.links.map((link, j) => (
                  <li key={j}>
                    <Link
                      href={link.href}
                      className="text-[13px] text-white/50 hover:text-white transition-colors duration-200 flex items-center gap-1 group"
                    >
                      <ChevronRight
                        size={11}
                        className="opacity-0 group-hover:opacity-100 -ml-1 transition-all duration-200 text-[#c4922a]"
                      />
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <input
                ...
                className="flex-1 md:w-[280px] bg-white/5 border border-white/10 text-white placeholder:text-white/25 text-[13px] px-4 py-3 rounded-xl outline-none focus:border-[#c4922a] transition-colors"
              />
        ...
        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {["Visa", "Mastercard", "UPI", "Razorpay", "COD"].map((method, i) => (
              <span
                key={i}
                className="text-[10px] font-semibold px-2.5 py-1 rounded-md bg-white/5 text-white/30 border border-white/10"
              >
                {method}
              </span>
            ))}
