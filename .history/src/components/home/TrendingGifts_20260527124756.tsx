...
import { Heart, ChevronLeft, ChevronRight, Sparkles, TrendingUp } from "lucide-react";
import { useWishlistStore } from "@/lib/store/wishlistStore";

interface Product {
  id: number; name: string; slug: string;
  price: number; comparePrice: number | null;
  images: string[]; badge: string | null; customizable: boolean;
}
...
export default function TrendingGifts() {
  ...
  useEffect(() => {
    fetch("/api/products?sort=newest&limit=12&status=ACTIVE")
      .then(r => r.json())
        ...
        <div className="flex gap-4">
          {Array.from({length:5}).map((_,i) => (
            <div key={i} className="flex-shrink-0 w-[220px]">
              ...
              <div className="h-4 bg-[#e8e0d5] rounded-full animate-pulse mb-2 w-3/4" />
    ...
    <section className="py-12 px-4 md:px-10">
      <div className="max-w-[1450px] mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-[26px] md:text-[32px] font-bold text-[#1a1a1a] flex items-center gap-3">
              <TrendingUp size={28} className="text-[#c0555a]" /> Trending now
            </h2>
            <p className="text-[13px] text-[#888] mt-0.5">What everyone is gifting right now</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/shop?sort=newest" className="text-[13px] text-[#c0555a] font-semibold hover:underline">View all →</Link>
        ...
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: "none" }}>
          {products.map(p => {
            ...
            return (
              <div key={p.id} className="flex-shrink-0 w-[220px] md:w-[240px] group relative">
                <Link href={`/product/${p.slug}`}>
                  <div className="relative aspect-square rounded-2xl overflow-hidden bg-[#f8f5f0] mb-3">
                    <Image src={p.images[0]} alt={p.name} fill
                      className="object-cover transition-transform duration-500 group-hover:scale-[1.04]" sizes="260px" />
                    {p.badge && (
                  ...
                  <p className="text-[13px] font-semibold text-[#1a1a1a] capitalize line-clamp-2 mb-1.5 group-hover:text-[#c0555a] transition-colors">{p.name}</p>
