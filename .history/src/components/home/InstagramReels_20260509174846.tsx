...
export default function InstagramReels() {
  ...
  // Navigate reels in modal
  const goNext = () => {
    if (activeModal === null) return;
    const next = (activeModal + 1) % reels.length;
    setActiveModal(next);
    setIsPlaying(true);
    setProgress(0);
    setLiked(false);
  ...
  const goPrev = () => {
    if (activeModal === null) return;
    const prev = (activeModal - 1 + reels.length) % reels.length;
    setActiveModal(prev);
    setIsPlaying(true);
    setProgress(0);
    setLiked(false);
      ...
      <section className="pt-0 pb-16 md:pb-24 relative overflow-hidden">
        <div className="max-w-[1450px] mx-auto px-4 md:px-6 lg:px-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10">
            <div>
              <span className="inline-block text-[#c4922a] text-lg md:text-xl italic mb-2 font-light" style={caveatFont}>
                as seen on instagram
              </span>
              <h2 className="text-5xl md:text-6xl font-bold text-[#1a1a1a]" style={caveatFont}>
                @hashtagifting
              </h2>
            </div>
                <a
          ...
          <div className="relative">
            <div className="absolute left-0 top-0 bottom-16 w-16 z-10 pointer-events-none"
              style={{ background: "linear-gradient(to right, var(--background), transparent)" }} />
            <div className="absolute right-0 top-0 bottom-16 w-24 z-10 pointer-events-none"
            ...
            <div ref={sliderRef} className="flex gap-4 overflow-x-auto scroll-smooth no-scrollbar pb-2">
              {reels.map((reel, index) => (
                <div key={index} className="flex-shrink-0 w-[220px] md:w-[240px] flex flex-col">
      ...
      {activeModal !== null && currentReel && (
        ...
        >
          ...
          >
            ...
            {/* VIDEO CARD — phone shape */}
            <div className="relative w-[320px] md:w-[360px] rounded-[28px] overflow-hidden bg-black shadow-2xl"
